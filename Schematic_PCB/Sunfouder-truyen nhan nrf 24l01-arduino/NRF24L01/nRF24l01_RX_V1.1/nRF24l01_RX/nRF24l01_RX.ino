...
void init_io(void)
{
  digitalWrite(IRQ, 0);  //中断信号。无线通信过程中MCU主要是通过IRQ与NRF24L01进行通信
  digitalWrite(CE, 0);	 // chip enable 芯片的模式控制线。在CSN为低的情况下，CE协同NRF24L0A的CONFIG寄存器共同决定NRF24L01的状态
  digitalWrite(CSN, 1);  // Spi disable 芯片片选线，CSN为低电平芯片工作	
}
